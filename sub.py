...
class Funshi:
    ...
    class FReView(discord.ui.View):
        def __init__(self, timeout: float | None = 180, disable_on_timeout: bool = False):
            super().__init__(timeout=timeout, disable_on_timeout=disable_on_timeout)
            
        @discord.ui.button(label="Edit", style=discord.ButtonStyle.primary)
        async def edit_callback(self, butotn, interaction:discord.Interaction):
            await interaction.response.send_modal()

        @discord.ui.button(label="Done", style=discord.ButtonStyle.green)
        async def done_callback(self, butotn, interaction:discord.Interaction):
            self.clear_items()
    ...
    class FSelect(discord.ui.Select):
        ...
        async def callback(self, interaction: discord.Interaction):
            data = Funshi.loading()
            await interaction.response.send_message(embed=Funshi.details_base(data[self.values[0]]))




    class FBSelect(discord.ui.Select):
        def __init__(self, placeholder:str, options: list[discord.SelectOption] = ... ) -> None:
            super().__init__(placeholder=placeholder, options=options)
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            #view.add_item(b)
            await interaction.response.send_message(embed=e, view=Funshi.FReView())
    ...
    class FView(discord.ui.View):
        def __init__(self, options: list[discord.SelectOption], timeout:int | None = 60, placeholder:str | None = None, mode:str="select"):
            ...
            elif mode == "edit-select-modal":
                self.add_item(Funshi.FBSelect(placeholder=placeholder, options=options))
    ...
    def check_in_list(__user:discord.Member, flag:bool=True):
        ...
        if (flag):
            ...
            if (__user.bot):
                __result = False
                __text:str = "This user is a Bot! Can't register it"
    ...
    def backup():
        if (os.path.exists(FW.main_file_path) and os.path.exists(FW.edit_log_path)):
            __flag:bool = False
            with open(FW.main_file_path, mode="r", encoding="utf-8") as f:
                __data = json.dumps(json.load(f), indent=4, ensure_ascii=False)

                with open(FW.main_file_path, mode="w", encoding="utf-8") as backup:
                    backup.write(__data)
                    __flag = True
            return (FW.main_file_path, FW.edit_log_path, __flag)
        else:
...
async def search_funshi_list(interaction:discord.Interaction):
    await interaction.response.send_message(view=Funshi.FView(placeholder="ユーザーを指定してください。Timeout=60s", options=Funshi.select_data()))
...
@bot.slash_command(name="funshi-edit", description="憤死者名簿のメンバーを編集")
async def funshi_edit(inter:discord.Interaction):
    data:list[discord.SelectOption] = []
    funshi_data:dict = Funshi.loading()
    for idx, user in enumerate(funshi_data.values()):
        data.append(discord.SelectOption(
    ...
    try:
        await inter.response.send_message(view=Funshi.FView(placeholder="ユーザーを指定してください。Timeout=60s", options=data, mode="edit-select"), ephemeral=True)
    except:
...
@bot.slash_command(name="tset_modal")
async def testing_modal(inter:discord.Interaction):
    await inter.response.send_message(view=Funshi.FView(mode="edit-select-modal", options=Funshi.select_data()))
